...
public class TileEntityLaserGun extends TileEntityAimedCannon
{
	...
	public void fire(World world, double[] xyz)
	{
		...
		for (float i = 0; i <= this.getMaxRange(); i += 0.5F)
		{
			...
			BreakEvent breakEvent = new BreakEvent(x, y, z, this.worldObj, id, meta, player);
	...
	private Block getAffectedID(Block id, int meta)
	{
		...
		if (id == Blocks.stone || id == Blocks.stonebrick || id == Blocks.sandstone) return Blocks.flowing_lava;
		...
		if (id == Blocks.gravel) return Blocks.cobblestone;
		if (id == Blocks.tallgrass || id == Blocks.web || id == Blocks.yellow_flower || id == Blocks.snow || id == Blocks.red_flower || id == Blocks.red_mushroom || id == Blocks.brown_mushroom || id == Blocks.deadbush || id == Blocks.wheat || id == Blocks.carrots || id == Blocks.potatoes || id == Blocks.vine || id == Blocks.melon_stem || id == Blocks.pumpkin_stem || id == Blocks.waterlily) return Blocks.air;
