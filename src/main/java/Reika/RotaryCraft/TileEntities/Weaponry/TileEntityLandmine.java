...
public class TileEntityLandmine extends TileEntitySpringPowered
{
	...
	private boolean checkForPlayer(World world, int x, int y, int z)
	{
		...
		for (int i = 0; i < in.size(); i++)
		{
			...
			if (e.onGround && !e.isSneaking()) return true;
		}
		return false;
	}
	...
	{
		// TODO gamerforEA code clear: if (chain) this.chainedExplosion(world, x, y, z);
		if (inv[1] != null && inv[2] != null && inv[3] != null && inv[4] != null)
		{
			boolean flag = true;
			for (int i = 1; i <= 4; i++)
			{
				if (!!ReikaItemHelper.matchStackWithBlock(inv[i], Blocks.tnt)) flag = false;
			}
			if (flag) this.maxPowerExplosion(world, x, y, z);
		}
		...
		if (flaming)
		{
			if (!world.isRemote) world.newExplosion(null, x + 0.5, y + 0.5, z + 0.5, power, true, true);
		}
		else if (!world.isRemote) world.createExplosion(null, x + 0.5, y + 0.5, z + 0.5, power, true); */
		...
		for (int i = 0; i < in.size(); i++)
		{
			...
			{
				if (!((EntityPlayer) e).capabilities.isCreativeMode) RotaryAchievements.LANDMINE.triggerAchievement((EntityPlayer) e);
			}
			// TODO gamerforEA code clear: e.attackEntityFrom(DamageSource.setExplosionSource(new Explosion(world, null, e.posX, e.posY, e.posZ, power)), (int) power * 4);
			e.addPotionEffect(new PotionEffect(Potion.blindness.id, 400, 0));
			e.addPotionEffect(new PotionEffect(Potion.confusion.id, 450, 5));
			if (poison) e.addPotionEffect(new PotionEffect(Potion.poison.id, 200, 0));
			/* TODO gamerforEA code clear:
	...
	private void getExplosionModifiers()
	{
		for (int i = 5; i <= 8; i++)
		{
			if (inv[i] != null)
			{
				if (inv[i].getItem() == Items.blaze_powder) flaming = true;
				if (inv[i].getItem() == Items.spider_eye) poison = true;
				if (ReikaItemHelper.matchStackWithBlock(inv[i], Blocks.tnt)) chain = true;
				if (ReikaItemHelper.matchStackWithBlock(inv[i], Blocks.glass)) shrapnel = true;
			}
		}
	}
	...
	private float getExplosionPower()
	{
		...
		for (int i = 1; i <= 4; i++)
			if (inv[i] != null)
			{
				if (inv[i].getItem() == Items.gunpowder) num++;
			}
		return 2F * num;
	}
	...
	public void openInventory()
	{
		...
		if (rand.nextInt(65536 - this.getAge()) / 2 == 0) this.detonate(worldObj, xCoord, yCoord, zCoord);
	}
	...
	{
		if (is.getItem() == Items.blaze_powder) return true;
		if (is.getItem() == Items.spider_eye) return true;
		if (ReikaItemHelper.matchStackWithBlock(is, Blocks.tnt)) return true;
		if (ReikaItemHelper.matchStackWithBlock(is, Blocks.glass)) return true;
		return false;
	}
	...
	private void chainedExplosion(World world, int x, int y, int z)
	{
		...
		for (int i = 0; i < 12; i++)
		{
			...
			if (!world.isRemote) world.spawnEntityInWorld(tnt);
		} */
	}
	...
	public void updateEntity(World world, int x, int y, int z, int meta)
	{
		...
		if (!DragonAPICore.debugtest && this.ageFail()) this.detonate(world, x, y, z);
		if (this.checkForArrow(world, x, y, z)) this.detonate(world, x, y, z);
		if (this.checkForPlayer(world, x, y, z)) this.detonate(world, x, y, z);
	}
