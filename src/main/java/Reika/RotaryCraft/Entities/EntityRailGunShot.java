...
public class EntityRailGunShot extends EntityTurretShot
{
	...
	public EntityRailGunShot(World world, double x, double y, double z, double vx, double vy, double vz, int pw, TileEntityRailGun r)
	{
		...
		motionY = vy;
		motionZ = vz;
		if (!world.isRemote) velocityChanged = true;
		power = pw;
		gun = r;
	}
	...
	public void onUpdate()
	{
		...
		if (ReikaWorldHelper.softBlocks(id) && !(id instanceof BlockLiquid || id instanceof BlockFluidBase) && ConfigRegistry.ATTACKBLOCKS.getState()) ReikaWorldHelper.recursiveBreakWithinSphere(worldObj, (int) Math.floor(posX), (int) Math.floor(posY), (int) Math.floor(posZ), id, -1, (int) Math.floor(posX), (int) Math.floor(posY), (int) Math.floor(posZ), 4);
		if (hit)
		{
			//ReikaChatHelper.write("HIT  @  "+ticksExisted+"  by "+(mobs.size() > 0));
			this.onImpact(null);
			if (power < 15 || true) this.setDead();
			return;
		}
		...
		if (!worldObj.isRemote && (shootingEntity != null && shootingEntity.isDead || !worldObj.blockExists((int) posX, (int) posY, (int) posZ))) this.setDead();
		else
		{
			...
			if (var3 != null) var2 = Vec3.createVectorHelper(var3.hitVec.xCoord, var3.hitVec.yCoord, var3.hitVec.zCoord);
			Entity var4 = null;
				...
				for (int k = -3; k <= 3; k++)
				{
					if (i * j * k < 9 && i * j * k > -9)
					{
						...
						if (ConfigRegistry.ATTACKBLOCKS.getState() && ConfigRegistry.RAILGUNDAMAGE.getState())
						{
							...
							if (ReikaWorldHelper.softBlocks(world, x0 + i, y0 + j, z0 + k) && !ReikaWorldHelper.isLiquidSourceBlock(worldObj, x0 + i, y0 + j, z0 + k)) ReikaWorldHelper.recursiveBreakWithinSphere(world, x0 + i, y0 + j, z0 + k, id, -1, x0 + i, y0 + j, z0 + k, 5);
							if (power >= 1)
							{
							...
							if (power >= 3)
							{
								...
								if (id == Blocks.grass)
								{
									...
									else world.setBlock(x0 + i, y0 + j, z0 + k, Blocks.dirt);
								}
								...
								if (id == Blocks.dirt)
								{
									...
									else world.setBlockMetadataWithNotify(x0 + i, y0 + j, z0 + k, meta + 1, 3);
								}
							}
							if (power >= 5)
							{
								if (id == Blocks.stone)
								{
									...
									{
										if (power <= 12) world.setBlock(x0 + i, y0 + j, z0 + k, Blocks.cobblestone);
										else
										{
											ReikaItemHelper.dropItems(world, x0 + i, y0 + j, z0 + k, id.getDrops(world, x0 + i, y0 + j, z0 + k, meta, 0));
	...
	{
		if (power == 15) return Integer.MAX_VALUE;
	...
	protected void applyAttackEffectsToEntity(World world, EntityLivingBase el)
	{
		...
		{
			if (el.isDead || el.getHealth() <= 0) RotaryAchievements.RAILKILLED.triggerAchievement((EntityPlayer) el);
		}
